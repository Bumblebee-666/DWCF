...
class TriModalTransformer(nn.Module):
    ...
    def __init__(self, args):
        ...
        boost_hidden_dim = max(embed_dim // 2, 64)
        self.boost_head_text = nn.Sequential(
            nn.Linear(embed_dim, boost_hidden_dim),
            nn.ReLU(),
            nn.Linear(boost_hidden_dim, n_classes)
        )
        ...
        if getattr(args, 'use_contrastive', False):
            self.projection_head = nn.Sequential(
                nn.Linear(embed_dim, max(embed_dim, 128)),
                nn.ReLU(),
                nn.Linear(max(embed_dim, 128), 128)
            )
    ...
    def get_fusion_output_from_features(self, t_feat, a_feat, v_feat):
        """从已有三模态特征重新计算融合输出。"""
        return self.fusion_head(t_feat, a_feat, v_feat)
